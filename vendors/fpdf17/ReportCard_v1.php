...
class ReportCard extends FPDF{
	...
	private function draw_hs_gs($subjects,$attendance,$conduct_tmplt,$student,$adviser,$section_alias,$dept_alias){
		...
		//Grades
		//Line Constants
		$grades_hlines_const['base'] = 1.61 + 0.39;
		...
		//Build grades 
		$x =  $grades_lines['v'][1] +(($grades_lines['v'][1]-$grades_lines['v'][2])*0.5);
		$this->build_grades($subjects, $disp_x, $x, $grades_lines, 'nomen','grades', 'under', array('A', 'F','NULL'));
		
		//Attendance
		//Line Constants
		$attend_hlines_const['base'] = 4.54 + 0.19;
		$attend_hlines_const['max'] = 4;
		$attend_hlines_const['height'] = 0.47 / 3;
		$attend_hlines_const['startX'] = 0.55;	
		$attend_hlines_const['endX']=0.55 +7.07; 
		$attend_vlines_const['base'] = 2.28;
		$attend_vlines_const['max'] = 12;
		...
		//Conduct
		//Line Constants
		$condct_hlines_const['base'] = 5.54 + 0.32;
		...
		//Build Lines
		$conduct_lines = $this->build_lines($condct_hlines_const, $condct_vlines_const, 0);
		if($this->showLines){$this->build_headers($conduct_lines['v'], $periods, $disp_y, $disp_until);}
		
		//Build Grades
		$x = $conduct_lines['v'][1] +(($conduct_lines['v'][1]-$conduct_lines['v'][2])*0.5);
		$this->build_grades($conduct_tmplt, $disp_x, $x, $conduct_lines, 'desc','score');
		...
		//Top
		$this->SetFont('Arial','',9); //Font
		if($this->showLines){$this->right_text(1.84,1.74,'Name:');}
		$this->Text(0.87,1.10,$student['fullname']);
		if($this->showLines){$this->right_text(1.84,1.88,'Section');}
		$this->Text(1.52,1.27,$section_alias);
		
		if($this->showLines){$this->right_text(1.84,2.02,'Class Adviser');}
		$this->Text( 1.47,1.44,$adviser['full_name']);
		$this->right_text(7.62,1.10,'2011-2012');
		...
		for($ctr=0;$ctr<count($lines['h'])-1;$ctr++){
			if($ctr<count($labels)){
				...
				$headers =  array();
					...
					$this->build_headers($lines['v'], $headers,  $lines['h'][$ctr+1]-$offset, 9);
			}
		}
