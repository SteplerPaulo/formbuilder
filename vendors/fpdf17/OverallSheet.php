...
class MasterSheet extends FPDF{
	...
	function create($subjects,$info, $data, $start_record_at,$record_max,$hasmaka){
		...
		$this->Rect(0.26+$adjust_leap,2.73-$adjust_hop,5.77,0.61);  // Header Box
		$this->Rect(6.16+$adjust_leap,2.73-$adjust_hop,0.83,0.61); // Final Box
		...
		$record_width = 5.77;
		$record_height = 0.17;
		$record_x = 0.26+$adjust_leap;
		$record_y = 3.34-$adjust_hop;
		$offset_x = 0.42;
		$offset_y = 0.03;
		$offset_name =0.7;
		$final_g_x = 6.16+$adjust_leap;
		$final_g_y = 3.34-$adjust_hop;
		$final_g_width = 0.83;
		$att_x = 7.04;
		$att_y = 3.34-$adjust_hop;
		$att_width = 1.09;
		$student_ctr=0;
		$this->line_ctr = 0;
		$curr_g_flg = '';
		...
		$line_start = 2.73-$adjust_hop;
		$line_end =3.17+( $record_height*($this->sheet_max_record-1))-$adjust_hop;
		$line_x = array(0.58+$adjust_leap);
		$cell_width =3.69/(count($subjects)+1);
		for($index = 0; $index<count($subjects)+2;$index++){
			array_push($line_x,((2.34 +$adjust_leap)+ ($cell_width*$index)));
		}
		array_push($line_x,6.16+$adjust_leap,6.56+$adjust_leap,6.99+$adjust_leap);	
		foreach($line_x as $x){
		...
		if($hasmaka){
		$this->Rect(2.34+($cell_width*4)+$adjust_leap,2.73-$adjust_hop,$cell_width*5,0.24,'FD'); // Makabayan Box
		}
		$this->Rect(6.16+$adjust_leap,2.73-$adjust_hop,0.83,0.61,'FD'); // Final Box
		...
		$this->Text((($this->paper_width/2 )-($this->GetStringWidth($SY.', '.$PERIOD)/2)),2,$SY.', '.$PERIOD);
	...
	//	$this->Text((5.62 + (2.38/2))-($this->GetStringWidth($PERIOD)/2),2.47,$PERIOD);
		...
		$this->Text(1.20,3.31-$adjust_hop,'NAME');
		$this->SetFont('Helvetica','',7);
		if($hasmaka){
		$this->Text((((2.34+($cell_width*4))+($cell_width*5/2))-($this->GetStringWidth($MAKABAYAN)/2))+$adjust_leap,2.92-$adjust_hop,$MAKABAYAN);
		}
		$this->SetFont('Helvetica','B',9);
		$this->Text(6.39+$adjust_leap,3.30-$adjust_hop,'FINAL');
		$this->SetFont('Helvetica','',7);
		$this->Text(7.22	,2.93,'');
		...
		foreach($subj_arr as $subj){
			...
			$this->Text($line_x[$ctr+1]-$offset,$subj_y-$adjust_hop ,$subj);
			$ctr+=1;
		}
		...
		for($ctr =0;$ctr<count($data);$ctr+=1){
			if($ctr<$record_max && $start_record_at<count($data)){
				...
				if(is_array($data[$ctr])){
					...
					$this->SetFont('Helvetica','',6);
					$student_rank=$data[$start_record_at][count($data[$start_record_at])-2];
					$this->line_ctr+=1;
					$disp_y = $record_y+($record_height*($this->line_ctr))-$offset_y;
					$disp_x =(0.24 +( $offset_x /2) ) - ($this->GetStringWidth(''.$student_ctr)/2);
					$this->Text($disp_x+$adjust_leap,$disp_y,$student_rank); // Number
					$this->Text($offset_name+$adjust_leap,$disp_y,utf8_decode($data[$start_record_at][2])); // Name
					//Plot grades
					for($subj_ctr=0;$subj_ctr<count($subjects);$subj_ctr+=1){
						
						$subj_grade = $data[$start_record_at][$subj_ctr+3];
