...
class KeysForm extends Formsheet{
	...
	function hdr($data){
		...
		$this->leftText(1,2,'Key Count : '.count($data['Key']),'','');
		$this->rightText(44,2,'Date Created : '.$date,'','');
	...
	function keys($data){
		...
		foreach($data['Key'] as $key){
			if($line2>63){
				$this->rightText(43,66,'Page '.$curr_page.' of '.count($data['Key'])/100,'');
				$curr_page++;
		...
		$total_page_count = count($data['Key'])/100;
		$this->rightText(43,66,'Page '.$curr_page.' of '.ceil(count($data['Key'])/100),'');
