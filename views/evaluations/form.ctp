			...
			<dl class="well"> 
				...
				<dd>
					...
					<? if($question_data['OptionType']['name'] == "textarea"){; ?>
						<dl class="textarea">
							<dd>
							<? echo $this->Form->input('EvaluationDetail.'. $index.'.answer',array('type'=>'textarea','class'=>'center w100','placeholder'=>'Fill out this area...','label'=>false)); ?>
							<? echo $this->Form->input('EvaluationDetail.'. $index.'.question_id',array('type'=>'hidden','value'=>$question_data['id'],'label'=>false)); ?>
							<? echo $this->Form->input('EvaluationDetail.'. $index++ .'.option_type',array('type'=>'hidden','value'=>'textarea','label'=>false)); ?>
							</dd>
						</dl><br/>
					<?}else if($question_data['OptionType']['name'] == "checkbox"){; ?>
						<dl class="options well">
							...
							<dd>
								<input type="checkbox" name="data[EvaluationDetail][<? echo $index;?>][option_id]" value="<? echo $option['Option']['id']; ?>"></input>
								<? echo $option['Option']['text']; ?>
								<? echo $this->Form->input('EvaluationDetail.'. $index .'.question_id',array('type'=>'hidden','value'=>$question_data['id'],'label'=>false)); ?>
								<? echo $this->Form->input('EvaluationDetail.'. $index++ .'.option_type',array('type'=>'hidden','value'=>'checkbox','label'=>false)); ?>
							</dd>
							<? endforeach;?>
						</dl>
					...
					<?}else if($question_data['OptionType']['name'] == "radio"){; ?>
						...
						<dl class="options well">
							<? foreach($question_data['QuestionOption'] as $option):?>
							<dd>
								<input type="radio" name="data[EvaluationDetail][<? echo $index;?>][option_id]" value="<? echo $option['Option']['id']; ?>"></input>
								<? echo $option['Option']['text']; ?>
							</dd>
							<? endforeach;?>
							<? echo $this->Form->input('EvaluationDetail.'. $index .'.question_id',array('type'=>'hidden','value'=>$question_data['id'],'label'=>false)); ?>
							<? echo $this->Form->input('EvaluationDetail.'. $index++ .'.option_type',array('type'=>'hidden','value'=>'radio','label'=>false)); ?>
						</dl>
					<? }; ?>
				</dd>
				<? endforeach;?>
