	...
	<body class="paper" server-date="<?php echo date('m-d-Y h:i:s');  ?>" server-year="<?php echo date('Y'); ?>" >
		...
		<nav class="main-nav animate" id="main-nav">
			...
			if($access->isLoggedIn()):
				echo $this->Html->link( $this->Html->tag('img', '', array('style'=>'width:40px;height:40px','class' => 'media-object','alt'=>'','src'=>(!empty($user['Document']['id']))?'formbuilder/users/download/'.$user['Document']['id']:'/bookstore/img/200x250.gif')).' '.
